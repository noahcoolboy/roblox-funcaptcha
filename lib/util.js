...
function get_bda() {
    return new Promise(async a => {
        ...
        let bda = [
            ...
            {"key": "wh", "value":`${murmurHash(fingerprints.windowhash.sort().join("|"), 420)}|${murmurHash(fingerprints.protohash.join("|"), 420)}`},
        ...
        let key = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Safari/537.36" + Math.round(time - time % 21600)
        let encrypted = await crypto.encrypt(JSON.stringify(bda), key)
        a(Buffer.from(encrypted).toString("base64"))
    })
}

module.exports = { apiBreakers, tileToLoc, get_bda }