...
function getToken(publicKey, blob) {
    return new Promise(async a => {
        a(JSON.parse(await http({
            ...
            method: "POST",
            body: `bda=${await util.get_bda()}&public_key=${publicKey}${blob ? `&data[blob]=${encodeURIComponent(blob)}` : ""}`,
            headers: {
...
function getFuncaptcha(token) {
    return new Promise(async a => {
        ...
        let captcha = await http({
            ...
            headers: { "User-Agent": util.USERAGENT, "Content-Type": "application/x-www-form-urlencoded" },
            body: `token=${captchaToken}&sid=${sid}`,
            method: "POST"
        })
...
function getDecryptionKey(captchaInfo) {
    return new Promise(async a => {
        let response = await http({
            ...
            body: `session_token=${captchaInfo.session_token}&game_token=${captchaInfo.challengeID}`
        })
        a(response)
    })
